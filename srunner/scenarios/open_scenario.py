...
from srunner.scenariomanager.scenarioatomics.atomic_behaviors import ChangeActorAgent, ChangeActorTargetSpeed
                        ...
                        if private.attrib.get('entityRef', None) == actor.rolename:
                            for private_action in private.iter("PrivateAction"):
                                for controller_action in private_action.iter('ControllerAction'):
                                    agent, args = OpenScenarioParser.get_controller(controller_action, self.config.catalogs)
                                    controller_atomic = ChangeActorAgent(carla_actor, agent=agent, args=args)
